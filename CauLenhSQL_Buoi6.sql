...
WHERE NHANVIEN.MANV = THANNHAN.MA_NVIEN AND
		  NHANVIEN.MANV IN (SELECT THANNHAN.MA_NVIEN
							 FROM NHANVIEN, THANNHAN
...
--14. Để liệt kê danh sách các đề án mà cả hai nhân viên Đinh Bá Tiến và Trần Thanh Tâm cùng làm, ta có thể sử dụng câu lệnh sau:
SELECT DISTINCT d.MADA
...
AND nv1.HONV + '' + nv1.TENLOT + '' + nv1.TENLOT = 'Đinh Bá Tiến' AND nv2.HONV + '' + nv2.TENLOT + '' + nv2.TENLOT = 'Trần Thanh Tâm';


--15. Để lấy danh sách những nhân viên (bao gồm mã nhân viên, họ tên, phái) làm việc trong mọi đề án của công ty, ta có thể sử dụng câu lệnh sau:
SELECT pc.MA_NVIEN, nv.HONV + '' + nv.TENLOT + '' + nv.TENLOT as 'Họ và tên', nv.Phai
